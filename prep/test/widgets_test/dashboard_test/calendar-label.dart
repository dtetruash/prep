...
void main() {
  ...
  group('DateTime is be formatted as dd mm yyyy', (){
    ...
    dateTimeList.forEach((map){
      test('Test ' + (dateTimeList.indexOf(map) + 1).toString(), () {
        expect(calendarLabel.dateTimeFormatter(map['rawDate']), map['stringDate']);
      });
    });
  });
  ...
  testWidgets('CalendarCard has correct title', (WidgetTester tester ) async {
    ...
    final titleFinder = find.text(calendarLabel.dateTimeFormatter(DateTime(2019,1,1,1,0,0,0,0)));
